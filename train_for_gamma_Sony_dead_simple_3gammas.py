...
checkpoint_dir = './gt_Sony_dead_simple_3gammas/'
result_dir = './gt_Sony_dead_simple_3gammas/'
...
def network(input):
    ...
    #16X16 SIZE IMAGE?
    conv10 = slim.conv2d(pool2, 12, [1, 1], rate=1, activation_fn=None, scope='g_conv10')
...
input_images_all = [[None] * 6000] * len(gammas)
...
for gamma_index in range(len(gammas)):
    for ind in range(len(train_ids)):
        ...
        input_image = np.array(gamma_curve)[gamma_index][input_image]
        #IndexError: arrays used as indices must be of integer (or boolean) type
        
        #LOOKS LIKE INPUT IS multiplied by 65536
        input_image = np.multiply(input_image, 1./65535.)
        input_images_all[gamma_index][ind] = input_image
        image_gamma = gammas[gamma_index]
        #input_image_copy = np.copy(input_image)
        #np.squeeze(input_image_copy)
        cnt += 1
        #if(ind ==0):
        print("min, max, mean, gamma, argmax: %.5f, %.5f, %.5f, %.5f, %d" % (np.min(input_image), np.max(input_image), np.mean(input_image), image_gamma, np.argmax(input_image)))
...
for epoch in range(lastepoch, 4001):
    ...
    if epoch > 2000:
        learning_rate = 1e-5
    assigned_image_gamma_indices = np.random.randint(0, 3, 6000)
    assigned_image_gammas = np.array(gammas)[assigned_image_gamma_indices]
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        # crop
        H = input_images_all[gamma_index][ind].shape[1]
        W = input_images_all[gamma_index][ind].shape[2]

        xx = np.random.randint(0, W - ps)
        yy = np.random.randint(0, H - ps)
        ##LOOKS LIKE INPUT IS multiplied by 65536!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!XXXXXXXXXXXX
        gamma_index = assigned_image_gamma_index
        input_patch = input_images_all[gamma_index][ind][:, yy:yy + ps, xx:xx + ps, :]
