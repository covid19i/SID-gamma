...
checkpoint_dir = './all_of_gt_Sony_GPU_efficient_flattened_3output_more_simpler_print/'
result_dir = './all_of_gt_Sony_GPU_efficient_flattened_3output_more_simpler_print/'
...
def network(input):
    ...
    fc1 = slim.fully_connected(flatten1, 100, scope='fc_1')
...
#out_gamma = tf.Print(out_gamma, [out_gamma])#HAVE TO SEE WHAT THIS DOES!!!!!!!!!!!!!XXXXXXXXXX
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if(cnt == 1):
            ...
            print("Epoch %d: batch %d: Loss=%.3f, Moving loss=%.3f, Time=%.3f" % (
                epoch, cnt, training_dataset_loss, moving_loss, time.time() - st))
