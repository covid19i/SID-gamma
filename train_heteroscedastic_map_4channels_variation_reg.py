...
checkpoint_dir = './heteroscedastic_sigma_map_4channels_variation_reg_checkpoint/'
...
if not os.path.isdir(result_dir):
    os.mkdir(result_dir)
    train_accuracy_path = result_dir  + 'train_accuracy.txt'
    train_accuracy_file = open(train_accuracy_path, 'a')
...
def network(input):
    ...
    conv1 = slim.conv2d(input, 4, [3, 3], rate=1, activation_fn=relu, scope='g_conv1_1')
    conv2 = slim.conv2d(conv1, 4, [3, 3], rate=1, activation_fn=relu, scope='g_conv2_1')

    conv3 = slim.conv2d(conv2, 4, [3, 3], rate=1, activation_fn=relu, scope='g_conv3_1')
    conv4 = slim.conv2d(conv3, 4, [3, 3], rate=1, activation_fn=relu, scope='g_conv4_1')
...
Var_reg_loss = tf.reduce_sum(tf.image.total_variation(out_noise_level_map))
G_loss = tf.losses.mean_squared_error(noise_level_map, out_noise_level_map) + Var_reg_loss
...
for epoch in range(lastepoch, final_epoch):
    ...
    for batch_id in range(no_of_batches):     
        ...
        if(DEBUG == 1 and ((epoch - lastepoch) % 200 == 0 or (epoch - lastepoch) < 3) and cnt == 1):
            print_noises = 1
            print("\n\n(mean,stddev), image[0], hetero noise[0], const noise[0], image[0]")
            for k in range(BATCH_SIZE):
                print(np.mean(input_patch[k]), np.std(input_patch[k]))
            print("input_patch", input_patch[0,ps//2:ps//2+2,ps//2:ps//2+2,1])
        ...
        heteroschedastic_sigma_s = np.random.uniform(0, heteroschedastic_sigma_s_max, BATCH_SIZE)
        heteroschedastic_sigma_s_map_transpose = np.full((ps,ps,1, BATCH_SIZE), heteroschedastic_sigma_s)
        heteroschedastic_sigma_s_map = np.transpose(heteroschedastic_sigma_s_map_transpose, (3, 0, 1, 2))
        heteroschedastic_noise_s = np.random.normal(0, 1, (BATCH_SIZE,ps,ps,D)) * heteroschedastic_sigma_s_map
        input_patch = np.multiply(input_patch, np.add(1.0,  heteroschedastic_noise_s))
        sigma_c = np.random.uniform(0,sigma_c_max, BATCH_SIZE)
        sigma_c_map_transpose = np.full((ps,ps, 1, BATCH_SIZE), sigma_c)
        sigma_c_map = np.transpose(sigma_c_map_transpose, (3, 0, 1, 2))
        noise_c = np.random.normal(0, 1, (BATCH_SIZE,ps,ps,D)) * sigma_c_map
        input_patch = np.add(input_patch, noise_c)
        if(print_noises == 1):
            print("heteroschedastic_sigma_s: ", heteroschedastic_sigma_s)
            print("sigma_c: ", sigma_c)
            print("heteroschedastic noise: ", np.multiply(heteroschedastic_noise_s[0,ps//2:ps//2+2,ps//2:ps//2+2,1], 
                             input_patch[0,ps//2:ps//2+2,ps//2:ps//2+2,1]))
            print("AWG noise: ", noise_c[0,ps//2:ps//2+2,ps//2:ps//2+2,1], "\n")
            print_noises = 0
        ...
        if epoch % save_freq == 0:
            ...
            text_file.write('\nEpoch %04d\tBatch %05d\n\n' % (epoch, batch_id))
            text_file.write('Predictions of Noise std dev: hetero, same variance:')
            #output_list = output * scale
            output_list = np.mean(output, axis=(1, 2))
            np.savetxt(text_file, output_list)
            noise_sigma_list = np.mean(noise_sigmas_feed, axis=(1, 2)).tolist()
            assert(np.shape(noise_sigma_list) == (BATCH_SIZE,2))
            text_file.write('Synthetic added std dev of noises: hetero, same variance:')
            noise_sigma_list = [(str(x[0])+", " + str(x[1])) for x in noise_sigma_list]
            s = "\n "
            s = s.join(noise_sigma_list)
            text_file.write(s)
            ...
            text_file.close()
            if(cnt == 1):
                print("Wrote predictions to ", file_path, "for batch ", cnt)
    ...
    if((epoch - lastepoch) % 200 == 2 or (epoch - lastepoch) < 3):
        print("Loss vector (slice for the first 10 images) at ", yy, xx)
