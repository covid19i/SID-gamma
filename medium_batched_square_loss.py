...
checkpoint_dir = './gt_Sony_medium_MSE_lowerLRat200_BNeverywhere/'
result_dir = './gt_Sony_medium_MSE_lowerLRat200_BNeverywhere/'
...
def network(input):
    ...
    fc2 = slim.fully_connected(bn1_fc, 3, scope='fc_2')
    return fc2
...
G_loss = tf.losses.mean_squared_error(gt_gamma_one_hot, out_gamma)
...
print("BATCH_SIZE", BATCH_SIZE, ",final_epoch", final_epoch, ",no_of_batches", no_of_batches, 
...
for epoch in range(lastepoch, final_epoch):
    ...
    if((epoch - lastepoch) % 100 == 1 or (epoch - lastepoch) < 10):
        print(g_loss)
