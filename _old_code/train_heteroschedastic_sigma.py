...
checkpoint_dir = './heteroschedastic_sigma_checkpoint/'
result_dir = checkpoint_dir
RAM_ALLOCATED = 18000#in MB
...
gt_noise_sigmas = tf.placeholder(tf.int32, [None, 2])#Just two sigmas number per image. batch size = 1 in this paper, BTW
out_noise_sigmas = resnet(in_image, 2, reuse=False)
...
g_loss = np.ones((len(train_fns), 2))
...
for epoch in range(lastepoch, final_epoch):
    ...
    for batch_id in range(no_of_batches):     
        ...
        if(DEBUG == 1 and (epoch - lastepoch) % 200 == 1 and cnt == 1):
        ...
        noise_sigmas = np.array([heteroschedastic_sigma_s.item()/heteroschedastic_sigma_s_max, 
                                 sigma_c/sigma_c_max])
        noise_sigmas = np.transpose(np.repeat(noise_sigmas[:, np.newaxis], BATCH_SIZE, axis=1), (1, 0))
        noise_sigmas_feed = noise_sigmas
        
        
        #print(np.shape(exposures_feed))
        _, G_current, output = sess.run([G_opt, G_loss, out_noise_sigmas],
                                        feed_dict={in_image: input_patch, gt_noise_sigmas: noise_sigmas_feed, lr: learning_rate})
        ...
        if(cnt == 1 and ((epoch - lastepoch) % 5 == 1 or (epoch - lastepoch) < 10) ):
    ...
    if((epoch - lastepoch) % 20 == 2 or (epoch - lastepoch) < 3):
        print("Loss vector (slice for the first 10 images)")
        print(g_loss[0:min(10,len(train_fns))])
