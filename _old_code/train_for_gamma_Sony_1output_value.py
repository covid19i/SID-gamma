...
checkpoint_dir = './all_of_gt_Sony_GPU_efficient_flattened_1output_value/'
result_dir = './all_of_gt_Sony_GPU_efficient_flattened_1output_value/'
...
def network(input):
    ...
    fc2 = slim.fully_connected(fc1, 1, scope='fc_2')
...
gt_gamma = tf.placeholder(tf.float32, [None, 1])#Just one gamma number per image. batch size = 1 in this paper, BTW
out_gamma = network(in_image)
G_loss = tf.reduce_mean(tf.abs(out_gamma - tf.squeeze(gt_gamma)))
...
for epoch in range(lastepoch, 4001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        if(DEBUG == 1 and epoch == lastepoch and cnt == 0):
            print("Training on index " + str(ind))
        assigned_image_gamma = assigned_image_gammas[ind]#gamma for the image
        #print("Gamma: " + str(assigned_image_gamma))
        assigned_image_gamma_index = gammas.index(assigned_image_gamma)#Should be same as in assigned_image_gamma_indices
        #out_image = gamma_curve[assigned_image_gamma_index][out_image]#Is it int?
        ...
        assigned_image_gamma = np.array([assigned_image_gamma])#conversion for the sake of gt_gamma
        assigned_image_gamma = np.expand_dims(assigned_image_gamma, axis = 0)#batch size = 1
