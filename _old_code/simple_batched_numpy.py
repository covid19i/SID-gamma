...
checkpoint_dir = './gt_Sony_simple_batched_filters/'
result_dir = './gt_Sony_simple_batched_filters/'
...
for epoch in range(lastepoch, final_epoch):
    ...
    for i in range(no_of_batches):
        ...
        # crop
        H = input_image.shape[0]#index could be just 0 instead of ind[0]
        ...
        assigned_image_gamma_index = np.transpose(np.array([assigned_image_gamma_index]))#conversion for the sake of gt_gamma
...
#https://www.tensorflow.org/versions/r1.15/api_docs/python/tf/name_scope
with tf.variable_scope('plots', auxiliary_name_scope=False) as scope_conv:
    #weights = tf.get_variable('weights')
    weights_retrieved = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="g_conv1_1")[0]#https://stackoverflow.com/questions/35678883/tensorflow-getting-variable-by-name
    #tf.image_summary('conv1/filters', weights, max_images=10)
    print(weights_retrieved.shape)#(3, 3, 4, 32)
    print(type(weights_retrieved))# = weights_retrieved.numpy()
