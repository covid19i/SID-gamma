...
checkpoint_dir = './gt_Sony_CNN5_FC2_no_log_low_exposure_all_images/'
...
# get train IDs
train_fns = glob.glob(gt_short_dir + '0*[0-9]_00_*.ARW')#0 in the beginning means training set
...
for ind in range(len(train_fns)):
    ...
    #if(ind ==0):
    if(ind < 20):
        print("min, max, mean, exposure, argmax: %.5f, %.5f, %.5f, %.5f, %d" % (np.min(input_image), np.max(input_image), np.mean(input_image), exposure, np.argmax(input_image)))
