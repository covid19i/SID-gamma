...
checkpoint_dir = './gt_Sony_FC3_exposure_101GB_random_subsets/'
...
all_train_fns = glob.glob(gt_short_dir + '0*[0-9]_*_*.ARW')#0 in the beginning means training set
...
all_train_ids = [int(os.path.basename(train_fn)[0:5]) for train_fn in all_train_fns]
all_train_exposures = [float(get_exposure_time(os.path.basename(train_fn))) for train_fn in all_train_fns]
for i in range(7):
    print(all_train_fns[i], all_train_ids[i], all_train_exposures[i])

print("Found " + str(len(all_train_ids)) + " images to train with\n")
...
if DEBUG == 1:
    save_freq = 1
    init_index = 0
    total_images = len(all_train_fns)
    ...
    train_ids = all_train_ids[init_index:(init_index+limit)]
    train_fns = all_train_fns[init_index:(init_index+limit)]
    train_exposures = all_train_exposures[init_index:(init_index+limit)]

print("Training on " + limit + " images only at any point of time.\n")
...
for epoch in range(lastepoch, final_epoch):
    ...
    for batch_id in range(no_of_batches):
        ...
        if(cnt < 1000 and ((epoch - lastepoch) < 10) ):
            training_dataset_loss = np.mean(g_loss[np.where(g_loss)])
    ...
    if((epoch - lastepoch) % 10 == 1 or (epoch - lastepoch) < 5):
