...
checkpoint_dir = './all_of_gt_Sony_GPU_efficient_flattened_3output_more_simpler_wider/'
result_dir = './all_of_gt_Sony_GPU_efficient_flattened_3output_more_simpler_wider/'
...
def network(input):
    ...
    pool1 = slim.max_pool2d(conv1, [4, 4], padding='SAME')
    conv2 = slim.conv2d(pool1, 32, [3, 3], rate=1, activation_fn=lrelu, scope='g_conv1_2')
    pool2 = slim.max_pool2d(conv2, [4, 4], padding='SAME')


    #No batch norm in this?????
    conv5 = slim.conv2d(pool2, 512, [3, 3], rate=1, activation_fn=lrelu, scope='g_conv5_1')
    pool5 = slim.max_pool2d(conv5, [4, 4], padding='SAME')
    conv5 = slim.conv2d(pool5, 512, [3, 3], rate=1, activation_fn=lrelu, scope='g_conv5_2')
    pool5 = slim.max_pool2d(conv5, [4, 4], padding='SAME')
    #16X16 SIZE IMAGE?
    conv10 = slim.conv2d(pool5, 12, [1, 1], rate=1, activation_fn=None, scope='g_conv10')
    ...
    #print(out.shape)
    #(?,)
    flatten1.set_shape([None, 12*2*2])
    fc1 = slim.fully_connected(flatten1, 10, scope='fc_1')
