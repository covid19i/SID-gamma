...
checkpoint_dir = './heteroscedastic_sigma_map_g_loss_full_res_BS16_ps16_checkpoint/'
...
for epoch in range(lastepoch, final_epoch):
    ...
    for batch_id in range(no_of_batches):     
        ...
        for k in range(BATCH_SIZE):
            #if(epoch - lastepoch == 1 and k == 2):
             #   print(g_loss[ind[k], yy:yy + ps, xx:xx + ps, :])
             #   print("assigned")
            #    print(abs((noise_sigmas_feed[k] - output[k])))
            #    print("coming from")
