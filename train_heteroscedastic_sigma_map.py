...
checkpoint_dir = './heteroscedastic_sigma_map_checkpoint/'
...
if(for_sony ==  1):
    H= 1424
    W = 2128
g_loss = np.ones((len(train_fns), 1, 1, 2))
...
for epoch in range(lastepoch, final_epoch):
    ...
    for batch_id in range(no_of_batches):     
        ...
        for k in range(BATCH_SIZE):
            g_loss[ind[k], 0,0, :] = abs(np.mean((noise_sigmas_feed[k] - output[k]), axis = (0,1)))
    ...
    if((epoch - lastepoch) % 20 == 2 or (epoch - lastepoch) < 3):
        ...
        print(g_loss[0:min(10,len(train_fns)), 0,0,:])
